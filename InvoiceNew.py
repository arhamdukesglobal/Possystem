...
class Invoice_Class:
    def __init__(self, root):
        ...
        self.content_frame.columnconfigure(1, weight=2)  # Middle column (customer, cart, calculator)
        ...
        Product_Frame1.rowconfigure(1, weight=1)
        ...
        Product_Frame2.grid(row=1, column=0, sticky="ew", padx=5, pady=5)
        Product_Frame2.columnconfigure(1, weight=1)

        lbl_search = Label(Product_Frame2, text="Search Product | By Name", 
                          font=("Aptos Display", self.scale_font_size(12), "bold"), 
                          bg="white", fg="black").grid(row=0, column=0, columnspan=3, sticky="w", padx=5, pady=2)
        
        lbl_search2 = Label(Product_Frame2, text="Product Name", 
                           font=("Aptos Display", self.scale_font_size(12), "bold"), 
        ...
        txt_search = Entry(Product_Frame2, textvariable=self.var_search, 
                          ...
                          bg="lightyellow")
        txt_search.grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        
        # Buttons frame for search
        button_frame = Frame(Product_Frame2, bg="white")
        button_frame.grid(row=1, column=2, sticky="e", padx=5, pady=2)
        
        btn_show_all = Button(button_frame, text="Show All", command=self.show, 
                             font=("Aptos Display", self.scale_font_size(10), "bold"), 
                             bg="#083531", fg="white", cursor="hand2")
        btn_show_all.pack(side=TOP, fill=X, pady=(0, 2))
        
        btn_Search = Button(button_frame, text="Search", command=self.search, 
                           font=("Aptos Display", self.scale_font_size(10), "bold"), 
        ...
        self.ProductTable = ttk.Treeview(Product_Frame3, 
                                         ...
                                         height=8)  # Fixed height for responsiveness
        ...
        # Configure column widths as percentages
        ...
        self.ProductTable.column("pid", width=40, anchor="center")
        self.ProductTable.column("Name", width=100, anchor="w")
        ...
        middle_frame.rowconfigure(1, weight=1)  # Calculator & Cart frame
        ...
        Cal_Cart_Frame = Frame(middle_frame, bd=4, relief=RIDGE, bg="white")
        Cal_Cart_Frame.grid(row=1, column=0, sticky="nsew", pady=(0, 5))
        
        Cal_Cart_Frame.columnconfigure(0, weight=1)
        Cal_Cart_Frame.columnconfigure(1, weight=1)
        Cal_Cart_Frame.rowconfigure(0, weight=1)

        #========Calculator Frame=====
        self.var_cal_input = StringVar()
        Cal_Frame = Frame(Cal_Cart_Frame, bd=4, relief=RIDGE, bg="white")
        Cal_Frame.grid(row=0, column=0, sticky="nsew", padx=(5, 2), pady=5)
        ...
        txt_cal_input = Entry(Cal_Frame, textvariable=self.var_cal_input, 
                             font=("Aptos Display", self.scale_font_size(18), "bold"), 
                             bd=5, relief=GROOVE, state='readonly', justify=RIGHT)
        txt_cal_input.grid(row=0, column=0, columnspan=4, sticky="ew", padx=2, pady=2)
        ...
        for i, row_buttons in enumerate(calc_buttons):
            for j, btn_text in enumerate(row_buttons):
                if btn_text == 'C':
                    btn = Button(Cal_Frame, text=btn_text, 
                                font=('arial', self.scale_font_size(14), 'bold'), 
                                command=self.clear_cal, 
                                bd=3, cursor="hand2", bg="orange")
                elif btn_text == '=':
                    btn = Button(Cal_Frame, text=btn_text, 
                                font=('arial', self.scale_font_size(14), 'bold'), 
                                command=self.perform_cal, 
                                bd=3, cursor="hand2", bg="lightgreen")
                else:
                    btn = Button(Cal_Frame, text=btn_text, 
                                font=('arial', self.scale_font_size(14), 'bold'), 
                                command=lambda t=btn_text: self.get_input(t), 
                                bd=3, cursor="hand2")
        ...
        Cart_Frame.grid(row=0, column=1, sticky="nsew", padx=(2, 5), pady=5)
        ...
        self.CartTable.column("Name", width=90, anchor="w")
        self.CartTable.column("Price", width=80, anchor="e")
        self.CartTable.column("Quantity", width=60, anchor="center")
    ...
    def update_font_sizes(self):
        ...
        # This is a simplified version - in production, you'd want to update each widget individually
        pass
    ...
    def update_table_column_widths(self):
        ...
        if hasattr(self, 'ProductTable'):
            ...
            self.ProductTable.column("pid", width=int(40 * scale_factor))
            self.ProductTable.column("Name", width=int(100 * scale_factor))
            ...
            if hasattr(self, 'CartTable'):
                self.CartTable.column("pid", width=int(40 * scale_factor))
                self.CartTable.column("Name", width=int(90 * scale_factor))
                self.CartTable.column("Price", width=int(80 * scale_factor))
                self.CartTable.column("Quantity", width=int(60 * scale_factor))
